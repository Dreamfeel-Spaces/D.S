<script lang="ts">
	...
	async function handleVerify() {
		...
		requested = false;
		try {
			const response = await axios.post(`/verify/svr`);
			if (response.data) {
				requested = true;
				requesting = false;
			}
	...
	let key: string = data.CAPTCHA_SITEKEY;
	...
	async function handleVerification() {
		verifying = true;
		try {
			const response = await axios.put(`/verify/svr`, { value });
			if (response.data) {
				verifying = false;
...
<div class="dark:bg-gray-900 absolute w-full h-screen " style="z-index: 200000;top:0; bottom:0">
	<div class="flex items-center justify-center min-h-screen p-5 bg-gray-600 min-w-screen">
		...
		>
			...
			{#if !requested}
				<h3 class="text-2xl">Thanks for signing up for Dreamfeel Spaces!</h3>
			...
			<div class="mt-4">
				{#if !requested}
					<form on:submit|preventDefault={onSubmit}>
						<button type="submit" class="px-2 py-2 text-blue-200 bg-blue-600 rounded"
							>{requesting ? 'Please wait...' : 'Send verification code'}</button
						>
					</form>
				{/if}
				...
				{#if requested}
					<div class=" h-64 py-3 rounded text-center">
						...
						<div class="flex justify-center my-4 dark:text-black ">
							<SvelteOtp numberOnly separator="-" placeholder="********" bind:value />
						</div>

						<!-- <div id="otp" class="flex flex-row justify-center text-center px-2 mt-5">
							<input
								class="m-2 dark:text-gray-800 border h-10 w-20 text-center form-control rounded"
								type="number"
								name="first"
								maxlength="1"
							/>
							<input
								class="m-2 border dark:text-gray-800 h-10 w-20 text-center form-control rounded"
								type="number"
								name="second"
								maxlength="1"
							/>
						...
						<button
							...
							class={`px-9 py-2 mt-2 text-blue-200 ${
								value.length !== 6 ? 'bg-gray-600' : 'bg-blue-600'
							}  rounded`}>{verifying ? 'Verifying...' : 'Verify'}</button
						>

						<div class="flex justify-center text-center mt-5">
							<button
								on:click={handleVerify}
								class="flex itebms-center text-blue-700 hover:text-blue-900 cursor-pointer"
								><span class="font-bold">Resend OTP</span><i
					...
					<p class="mt-4 text-sm">
						If you haven't seen the email with the verification code, please check your spam or junk
						folder.
					</p>
				{/if}
			</div>
