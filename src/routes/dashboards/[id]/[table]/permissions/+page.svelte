<script lang="ts">
	...
	let permissions = roles.map((role) => {
		for (let cP of table.createPermissions) {
			if (cP.userRoles.id === role.id || role.isSuperUser) {
				role = { ...role, createChecked: true };
			}
		}

		for (let cP of table.viewPermissions) {
			if (cP.userRoles.id === role.id || role.isSuperUser) {
				role = { ...role, readChecked: true };
			}
		}

		for (let cP of table.updatePermissions) {
			if (cP.userRoles.id === role.id || role.isSuperUser) {
				role = { ...role, updateChecked: true };
			}
		}

		for (let cP of table.deletePermissions) {
			if (cP.userRoles.id === role.id || role.isSuperUser) {
...
<form method="post" class="max-h-99 overflow-auto">
	<div class="p-6">
		<div class="text-3xl mb-3 text-gray-500">Dashboard Permissions</div>
